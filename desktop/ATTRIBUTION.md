...
| `main/activities.js` | rewritten | Replaced FIL-specific spark/voyager online tracking with provad-aware online state (supervisor + provad/engine/httpserver/daemon subsystems flip the flag). |
...
| `main/preload.js` | rewritten | Channels renamed to `prova:*`. Renderer surface fully retyped: wallet, prover stats, lifecycle, updater normalization. |
...
| `main/station-config.js` | renamed/rewritten | Now `main/prova-config.js`; keys re-prefixed `prova-*`. |
...
| `main/test/wallet.test.js` | replaced | Old FIL-RPC wallet tests deleted; current coverage in `test/smoke.test.js` (ethers v6, BIP-39, EIP-191, deterministic keystore passphrase). |
| `renderer/` | overhauled | Stripped FIL UI; built fresh React 19 + Tailwind dashboard against the `prova:*` IPC bridge (wallet address, deals active, proofs submitted, activity feed, uptime, updater status). Vite + TS strict, builds clean. |
| `shared/typings.ts` | rewritten | Removed `FILTransaction*` and `BeryxTransaction`. Now exports only the cross-process `Activity` and `WalletSeed` types. |
