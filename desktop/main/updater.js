...
function onUpdateNotAvailable ({ version }) {
  ...
  showDialogSync({
    ...
    message: `You are on the latest version of Prova (${version}).`,
    type: 'info',
    buttons: ['Close']
  })
}
...
function onUpdateDownloaded (ctx, { version /*, releaseNotes */ }) {
  ...
  const showUpdateDialog = () => {
    const buttonIx = showDialogSync({
      title: 'Update Prova',
      message: `An update to Prova ${version} is available. ` +
  ...
  if (checkingManually) {
    ...
    updateNotification = new Notification({
      title: 'Prova Update',
      body: `An update to Prova ${version} is available.`
    })
