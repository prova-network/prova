...
/**
 * Write a minimal prover.toml at `STATE_ROOT/prover.toml`. Provad's
 * config struct (prover/pkg/config/config.go) reads `[storage] data_dir`
 * for the piece store; everything else can default. The desktop owns
 * this file end-to-end, so we always overwrite.
 */
async function ensureProverConfig () {
  ...
  // Plain string templating is fine here; we control every value.
  // Quoting on Windows paths: TOML allows backslashes inside double
  // quotes only when escaped, so use forward slashes throughout.
  ...
  log.info(format('wrote prover.toml: data_dir=%s', safeDataDir))
}
...
async function start (ctx) {
  ...
  // (storage location, prover/client mode toggles) take effect on the
  // next supervisor restart without a manual edit.
  await ensureProverConfig()
